...
public class MessageViewModel extends AndroidViewModel
    ...
    public void onWarnClick(final UiMessage uiMessage, Context context) {
        ...
        if (msg.getContent() != null) {
            if (msg.getContent() instanceof MediaMessageContent) {
                if (uiMessage.getExpansion()== null) {
                    if (((MediaMessageContent) msg.getContent()).getMediaUrl() != null) {
                        if (msg.getContent().getExtra() != null && !msg.getContent().getExtra().equals("")) {
                            Map map = new HashMap();
                            map = new Gson().fromJson(msg.getContent().getExtra(), map.getClass());
                            if (Objects.requireNonNull(map.get("isPrivate")).toString().equals("true")) {
                                HashMap<String, String> expansionKey = new HashMap<>();
                                expansionKey.put("isPrivate", "true");
                                msg.setExpansion(expansionKey);
                                msg.setCanIncludeExpansion(true);
                            }
                        }
        ...
        if (!isMember) {
            ...
            if (messagesNumber <= 0) {
                HttpRequest.INSTANCE.showBuyMember(context, 0,mConversationIdentifier.getTargetId());
                return;
            }
        }
        reSendMessage(msg);
                                ...
                                .actionListener(
                                        new MessageItemLongClickAction
                                            ...
                                            public boolean onMessageItemLongClick(
                                                ...
                                                enterEditState();
                                                onViewClick(MessageClickType.EDIT_CLICK, message,context);
                                                return true;
                                            }
                                        })
                                .showFilter(
    ...
    public void onViewClick(int clickType, UiMessage data, Context context) {
