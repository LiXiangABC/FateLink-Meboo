...
public class SendMediaManager {
    ...
    public void sendMedia(
            Context context,
            LocalMedia image, ConversationIdentifier conversationIdentifier,
            Uri mediaUri,
            long duration, boolean isPrivate, boolean sendAsPrivate) {
        if (!TextUtils.isEmpty(mediaUri.toString())) {
//            if (!FileUtils.isFileExistsWithUri(context, mediaUri)) {
//                return;
//            }
            SightMessage sightMessage;
            if (FileUtils.isHttp(mediaUri.getScheme())){
                Log.e(TAG, "sendMedia: "+duration );
                sightMessage = new SightMessage();
                sightMessage.setDuration((int) duration / 1000);
                sightMessage.setThumbUri(mediaUri);
                sightMessage.setMediaUrl(mediaUri);
            }else {
                sightMessage = SightMessage.obtain(context, mediaUri, (int) duration / 1000);
            ...
            sightMessage.setExtra(hashMap.toString());
    ...
    private class UploadController implements Runnable {
        ...
        public void run() {
            ...
            boolean b = ((SightMessage) executingMessage.getContent()).getMediaUrl() == null;
            if (b) {
                IMCenter.getInstance()
                        .sendMediaMessage(
                                executingMessage,
                                    ...
                                    public void onError(
                                        ...
                                        if (errorCode.code
                                            ...
                                            Toast.makeText(
                                                    ...
                                                    .show();
                                            return;
                                        }else if (errorCode.code == IRongCoreEnum.CoreErrorCode.RC_SIGHT_MSG_DURATION_LIMIT_EXCEED
                                                .getValue()){
                                            Toast.makeText(IMCenter.getInstance().getContext(), IMCenter.getInstance().getContext().getString(R.string.video_too_big), Toast.LENGTH_SHORT).show();
                                            return;
                                        }
                                        polling();
                                    }
