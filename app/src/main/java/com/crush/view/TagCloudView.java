...
public class TagCloudView extends ViewGroup {
    ...
    private static final int DEFAULT_TEXT_BACKGROUND = R.drawable.shape_solid_black_radius_6;
    ...
    public TagCloudView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        textPaddingLeft = DensityUtil.dip2px(context, a.getInteger(R.styleable.TagCloudView_tcvTextPaddingLeft, 6));
        textPaddingRight = DensityUtil.dip2px(context, a.getInteger(R.styleable.TagCloudView_tcvTextPaddingRight, 6));
        textPaddingTop = DensityUtil.dip2px(context, a.getInteger(R.styleable.TagCloudView_tcvTextPaddingTop, 4));
        textPaddingBottom = DensityUtil.dip2px(context, a.getInteger(R.styleable.TagCloudView_tcvTextPaddingBottom, 4));
        ...
        mFontFamily = a.getResourceId(R.styleable.TagCloudView_tcvFontFamily, -1);
        ...
        mTwoLine = a.getBoolean(R.styleable.TagCloudView_tcvTwoLine, DEFAULT_SINGLE_LINE);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mSingleLine) {
            ...
            totalHeight = getTwoTotalHeight(totalWidth, totalHeight);
        } else {
    ...
    private int getTwoTotalHeight(int totalWidth, int totalHeight) {
        int childWidth;
        int childHeight;
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (i == 0) {
                totalHeight = childHeight + mViewBorder;
                singleHeight = childHeight + mViewBorder;
            }

            if (totalWidth + mTagBorderHor + mViewBorder > sizeWidth) {
                totalWidth = mViewBorder;
                if (singleHeight*2 > totalHeight) {
        ...
        if (tags != null && tags.size() > 0) {
            for (int i = 0; i < tags.size(); i++) {
                ...
                if (mTagResId == DEFAULT_TAG_RESID) {
                    ...
                    if (mFontFamily != -1) {
                        Typeface fontFamilyTypeface = ResourcesCompat.getFont(tagView.getContext(), mFontFamily);//初始化字体
                        tagView.setTypeface(fontFamilyTypeface);
                    }
                }
                LayoutParams layoutParams = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
    ...
    public void setTagBeansCheck(List<TagBean> tagList) {
        ...
        if (tagBeans != null && tagBeans.size() > 0) {
            for (int i = 0; i < tagBeans.size(); i++) {
                ...
                tagView.setBackgroundResource(R.drawable.shape_solid_black_radius_4);
    ...
    public void setTagBeans(List<TagBean> tagList, boolean check) {
        ...
        if (tagBeans != null && tagBeans.size() > 0) {
            for (int i = 0; i < tagBeans.size(); i++) {
                ...
                if (check) {
                    tagView.setBackgroundResource(tagBeans.get(i).getCheck() ? R.drawable.shape_interest_select_bg : R.drawable.shape_interest_unselect_bg);
                }
                LayoutParams layoutParams = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
                tagView.setLayoutParams(layoutParams);
                tagView.setText(tagBeans.get(i).getInterest());
    ...
    public void getTags(int position, int resId) {
        TextView tagView = (TextView) getChildAt(position);
...
//        requestLayout();
    ...
    public void twoLine(boolean mTwoLine) {
        this.mTwoLine = mTwoLine;
        this.setTags(tags);
//        requestLayout();
    }
