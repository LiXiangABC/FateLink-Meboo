package com.crush.view.cardstackview;
...
import com.crush.view.cardstackview.internal.CardStackSetting;
import com.crush.view.cardstackview.internal.CardStackSmoothScroller;
import com.crush.view.cardstackview.internal.CardStackState;
import com.crush.view.cardstackview.internal.DisplayUtil;
...
public class CardStackLayoutManager
    ...
    private MutableLiveData<Integer> remainingBrowseBNumLive = new MutableLiveData<>();
    private MutableLiveData<Integer> remainingWlmNumLive = new MutableLiveData<>();

    public CardStackLayoutManager(Context context) {
        this(CardStackListener.DEFAULT,null,null);
    }

    public CardStackLayoutManager(CardStackListener listener, MutableLiveData<Integer> remainingBrowseBNum, MutableLiveData<Integer> remainingWlmNumLive) {
        this.context = Activities.Companion.get().getTop();
        this.listener = listener;
        this.remainingBrowseBNumLive = remainingBrowseBNum;
        this.remainingWlmNumLive = remainingWlmNumLive;
    ...
    public int scrollHorizontallyBy(int dx, RecyclerView.Recycler recycler, RecyclerView.State s) {
        ...
        switch (state.status) {
            ...
            case Dragging:
                if (setting.swipeableMethod.canSwipeManually()) {
                    state.dx -= dx;
                    if (BaseConfig.Companion.getGetInstance().getBoolean(SpName.INSTANCE.getUserCard() + getTopPosition(), false)) {
                        if (!BaseConfig.Companion.getGetInstance().getBoolean(SpName.INSTANCE.isMember(), false)) {
                            if (state.dx < 0) {
                                int bNum = remainingBrowseBNumLive.getValue() == null ? 0 : remainingBrowseBNumLive.getValue();
                                if (bNum <= 0) {
                                    MemberDialogShow.INSTANCE.memberBuyShow(2, context);
                                    state.dx = -50;
                                }
                            } else if (state.dx > 0) {
                                int wlmNum = remainingWlmNumLive.getValue() == null ? 0 : remainingWlmNumLive.getValue();
                                if (wlmNum <= 0) {
                ...
                if (state.targetPosition == RecyclerView.NO_POSITION) {
                    ...
                    if (state.topPosition < state.targetPosition) {
                        ...
                        if (BaseConfig.Companion.getGetInstance().getBoolean(SpName.INSTANCE.getUserCard() + getTopPosition(), false)) {
                            if (!BaseConfig.Companion.getGetInstance().getBoolean(SpName.INSTANCE.isMember(), false)) {
                                if (state.dx < 0) {
                                    if (BaseConfig.Companion.getGetInstance().getInt(SpName.INSTANCE.getRemainingBrowseBNum(), 0) <= 0) {
                                        state.next(CardStackState.Status.Idle);
                                        state.targetPosition = RecyclerView.NO_POSITION;
                                        break;
                                    }
                                } else if (state.dx > 0) {
                                    if (BaseConfig.Companion.getGetInstance().getInt(SpName.INSTANCE.getRemainingWlmNum(), 0) <= 0) {
                                        state.next(CardStackState.Status.Idle);
                                        state.targetPosition = RecyclerView.NO_POSITION;
                                        break;
                                    }
    ...
    private void updateRewind(RecyclerView.Recycler recycler) {
        state.width = getWidth();
        state.height = getHeight();
...
//        }
        ...
        //手动更改卡片是否喜欢的不透明度
        float alpha = setting.overlayInterpolator.getInterpolation(state.getRatio()) * 1.2f;
        switch (direction) {
            case Left:
                if (leftOverlay != null) {
