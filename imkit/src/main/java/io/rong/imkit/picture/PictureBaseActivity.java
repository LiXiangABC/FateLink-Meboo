...
//        colorPrimaryDark = AttrsUtils.getTypeValueColor(this, R.attr.colorPrimaryDark);
        Log.e(TAG, "initConfig: "+colorPrimary+"---"+colorPrimaryDark);
     ...
     * @param result
     */
    protected void handlerResult(List<LocalMedia> result,boolean isPrivate,boolean sendPrivate) {
        onResult(result,isPrivate,sendPrivate);
    }
     ...
     * @param sendPrivate
     */
    protected void onResult(List<LocalMedia> images, boolean isPrivate, boolean sendPrivate) {
        if (images == null) {
            return;
        }
        if (config.camera
        ...
        intent.putExtra(PictureConfig.PRIVATE_PHOTO_PREVIEW,isPrivate);
        intent.putExtra(PictureConfig.PRIVATE_PHOTO_PREVIEW_TRANSFORM,sendPrivate);
