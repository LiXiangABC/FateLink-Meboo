...
public class PicturePreviewActivity extends PictureBaseActivity
    ...
    private LinearLayout privatePhotoTitleContainer;
...
//    protected CheckBox mCbOriginal;
    ...
    private boolean is_private_photo;
    private LinearLayout sendAsPrivateContainer;
    private CheckBox cbSendAsPrivate;
    ...
    protected void initWidgets() {
        ...
        privatePhotoTitleContainer = findViewById(R.id.private_photo_title_container);
        picture_left_back = findViewById(R.id.picture_left_back);
        topLayout = findViewById(R.id.fl_top);
        viewPager = findViewById(R.id.preview_pager);
        btnCheck = findViewById(R.id.btnCheck);
        check = findViewById(R.id.check);
        cbSendAsPrivate = findViewById(R.id.cb_send_as_private);
        sendAsPrivateContainer = findViewById(R.id.send_as_private_container);
...
//        mCbOriginal = findViewById(R.id.cb_original);
        ...
        is_private_photo = getIntent().getBooleanExtra(PictureConfig.PRIVATE_PHOTO_PREVIEW, false);
        // 底部预览按钮过来
        images.clear();
        privatePhotoTitleContainer.setVisibility(is_private_photo ? View.VISIBLE : View.GONE);
        sendAsPrivateContainer.setVisibility(BaseConfig.Companion.getGetInstance().getBoolean("privatePhotoShowUnableFlag",false)?View.GONE:is_private_photo ? View.GONE : View.VISIBLE);
        if (is_bottom_preview) {
            images.addAll(getIntent().<LocalMedia>getParcelableArrayListExtra(PictureConfig.EXTRA_PREVIEW_SELECT_LIST));
        } else {
...
//            mCbOriginal.setVisibility(View.GONE);
        ...
        String mimeType = images.get(position).getMimeType();
        FirebaseEventUtils.INSTANCE.logEvent(is_private_photo?mimeType.contains("video")?FirebaseEventTag.Chat_PrivateAlbum_V.name():FirebaseEventTag.Chat_PrivateAlbum_P.name():mimeType.contains("video")?FirebaseEventTag.Chat_Album_V.name():FirebaseEventTag.Chat_Album_P.name());

        initViewPageAdapterData();
        // 原图
        LocalMedia media = images.get(position);

        sendAsPrivateContainer.setOnClickListener(view -> {
            cbSendAsPrivate.setChecked(!cbSendAsPrivate.isChecked());
        });
        cbSendAsPrivate.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                FirebaseEventUtils.INSTANCE.logEvent(cbSendAsPrivate.isChecked()?mimeType.contains("video")?FirebaseEventTag.Chat_Album_V_asPrivate.name():FirebaseEventTag.Chat_Album_P_asPrivate.name():mimeType.contains("video")?FirebaseEventTag.Chat_Album_V_noPrivate.name():FirebaseEventTag.Chat_Album_P_noPrivate.name());
            }
        });
...
//                ContextCompat.getDrawable(this, R.drawable.rc_picture_original_checkbox));
    ...
    @Override
    public void onResult(List<LocalMedia> images, boolean isPrivate, boolean sendPrivate) {
        if (is_private_photo){
            FirebaseEventUtils.INSTANCE.logEvent(images.get(position).getMimeType().contains("video")?FirebaseEventTag.Chat_PrivateAlbum_V_send.name():FirebaseEventTag.Chat_PrivateAlbum_P_send.name());
        }else {
            FirebaseEventUtils.INSTANCE.logEvent(cbSendAsPrivate.isChecked() ? images.get(position).getMimeType().contains("video") ? FirebaseEventTag.Chat_Album_V_asPrivate_send.name() : FirebaseEventTag.Chat_Album_P_asPrivate_send.name() : images.get(position).getMimeType().contains("video") ? FirebaseEventTag.Chat_Album_V_Send.name() : FirebaseEventTag.Chat_Album_P_Send.name());
        }
        Bundle bundle = new Bundle();
        bundle.putBoolean(PictureConfig.PRIVATE_PHOTO_PREVIEW_TRANSFORM,cbSendAsPrivate.isChecked());
        bundle.putBoolean(PictureConfig.PRIVATE_PHOTO_PREVIEW, cbSendAsPrivate.isChecked() || is_private_photo);
...
//                                + ")");
    ...
    public void onClick(View view) {
        ...
        if (id == R.id.picture_left_back) {
            ...
            if (cbSendAsPrivate.isChecked()){
                HttpRequest.INSTANCE.getUserInfoEmpty(getContext(), this::onComplete);
            }else {
                onComplete();
            }
    ...
    protected void onComplete() {
        ...
        onResult(selectImages,false, false);
    }
