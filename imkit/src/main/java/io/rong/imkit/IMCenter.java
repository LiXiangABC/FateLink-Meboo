...
public class IMCenter {
    ...
    private static final int MAX_BLOCK_SEND = 3000;
    private long LAST_SEND_SUCCESS_TIME = 0;
    ...
    public static void init(Application application, String appKey, InitOption option) {
        ...
        ArrayList<Class<? extends MessageContent>> messagesList = new ArrayList<>();
        messagesList.add(CombineMessage.class);
        RongIMClient.registerMessageType(messagesList);
    }
        ...
        RongIMClient.getInstance()
                .insertIncomingMessage(
                        ...
                        new RongIMClient.ResultCallback<Message>() {
                            @Override
                            public void onSuccess(Message message) {
                                LAST_SEND_SUCCESS_TIME = message.getSentTime();
                                for (MessageEventListener item : mMessageEventListeners) {
                                    item.onInsertMessage(new InsertEvent(message));
                                }
                                if (resultCallback != null) {
    ...
    public Context getContext() {
        return mContext==null? Activities.Companion.get().getTop():mContext;
    }
    ...
    private void filterSentMessage(
        ...
        if (errorCode != null
            ...
            if (errorCode.equals(RongIMClient.ErrorCode.NOT_IN_DISCUSSION)
                ...
                //判断当前发送时间是否大于最后一次发送时间+3秒
                if (message.getSentTime() > LAST_SEND_SUCCESS_TIME + MAX_BLOCK_SEND) {
                    Message.ReceivedStatus receivedStatus =
