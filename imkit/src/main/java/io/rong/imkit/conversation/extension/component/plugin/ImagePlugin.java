...
public class ImagePlugin implements IPluginModule, IPluginRequestPermissionResultCallback {
    ...
    public void getConversationIdentifier(ConversationIdentifier conversationIdentifier) {
        ...
        if (resultCode == RESULT_OK) {
            ...
            if (selectList != null && selectList.size() > 0) {
                ...
                for (LocalMedia item : selectList) {
                    ...
                    if (conversationIdentifier !=null && conversationIdentifier.getTargetId() != null) {
                        boolean isMember = BaseConfig.Companion.getGetInstance().getBoolean(conversationIdentifier.getTargetId() + "_member", false);
                        if (!isMember) {
                            int messagesNumber = BaseConfig.Companion.getGetInstance().getInt(conversationIdentifier.getTargetId() + "_messagesNumber", 0);
                            if (messagesNumber <= 0) {
                        ...
                        if (mimeType.startsWith("image")) {
                            SendImageManager.getInstance()
                                    .sendImage(conversationIdentifier, item, sendOrigin, data.getBooleanExtra(PictureConfig.PRIVATE_PHOTO_PREVIEW, false));
                            ...
                            SendMediaManager.getInstance()
                                    .sendMedia(
                                            IMCenter.getInstance().getContext(), item,
                                            conversationIdentifier,
                                            path,
                                            item.getDuration(), data.getBooleanExtra(PictureConfig.PRIVATE_PHOTO_PREVIEW, data.getBooleanExtra(PictureConfig.PRIVATE_PHOTO_PREVIEW, false)), data.getBooleanExtra(PictureConfig.PRIVATE_PHOTO_PREVIEW_TRANSFORM, false));
                            if (conversationIdentifier
                                    .getType()
                                    .equals(Conversation.ConversationType.PRIVATE)) {
                                RongIMClient.getInstance()
